...
#fname7 = "/Users/alexanderwoxstrom/Forskningsprojektet Rays/sofie+alex/cells/" + "20171207_amoeba8-01.tif" - File not fit for analysis
...
fnamesT0 = [fname6,fname8,fname9,fname10]
for cell in fnamesT0:
     [nVirus, conc, allLengths, area] = analyzeCrossImage(cell)
...
#area plots:
areaVsTimeMean = [areasT0.mean(), areasT6.mean(), areasT12.mean(), areasT18.mean(), areasT24.mean()]
areaVsTimeStd = [areasT0.std(), areasT6.std(), areasT12.std(), areasT18.std(), areasT24.std()]
...
#Calculate Hist - T12
histLenT12 = np.concatenate(lengthsT12)*pxSize
sortedLenT12 = histLenT12.std
lMin = 50 # nm
lMax = 1650 # nm
dL = 100 # nm
plt.show()
hist_bins = np.linspace(lMin, lMax, num=np.int((lMax - lMin)/dL + 1))
hist, hist_bins = np.histogram(np.concatenate(lengthsT12).astype(int)*pxSize, bins=hist_bins)
hist_bins_center = np.array([(hist_bins[i] + hist_bins[i+1])/2 for i in range(len(hist_bins) - 1)])
plt.bar(hist_bins_center, hist, 0.8*dL)
plt.xlabel('Different sizes (nm)')
plt.ylabel('Number of particles')
...
#plot areaVsTimeMean
plt.errorbar(infTimes, areaVsTimeMean, areaVsTimeStd, 0,"-or", capsize=5)
plt.xlabel('Time (h)')
plt.ylabel('Mean area of cells')
#plt.savefig('/Users/alexanderwoxstrom/Forskningsprojektet Rays/alex programmering/Plots/nVirusVsTime')
plt.show()

#print std and mean for the histograms
lT0 = np.concatenate(lengthsT0)*pxSize
lenT0Std = lT0.std()
lenT0Mean = lT0.mean()
print(lenT0Std)
print(lenT0Mean)
...
lT12 = np.concatenate(lengthsT12)*pxSize
lenT12Std = lT12.std()
lenT12Mean = lT12.mean()
print(lenT12Std)
print(lenT12Mean)


lT24 = np.concatenate(lengthsT24)*pxSize
lenT24Std = lT24.std()
lenT24Mean = lT24.mean()
print(lenT24Std)
print(lenT24Mean)
